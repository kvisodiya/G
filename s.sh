#!/bin/bash
##############################################################################
# Fresh Debian → Lynis 90+ (SAFE - nothing breaks)
# 
# Tested safe:
#   - Internet works
#   - SSH works  
...
BACKUP="/root/pre-harden-backup_$(date +%s)"
...
echo ""
echo "╔═══════════════════════════════════════╗"
echo "║  Fresh Debian → Lynis 90+ (Safe)     ║"
...
if sshd -t 2>/dev/null; then
  ...
  systemctl restart ssh 2>/dev/null || systemctl restart sshd 2>/dev/null
  echo "  SSH reverted (safe)"
fi

########################################
# STEP 3: FIREWALL (UFW only - no iptables mess)
########################################
echo "[3/15] Firewall..."
...
########################################
# STEP 5: KERNEL HARDENING (safe sysctl)
########################################
echo "[5/15] Kernel hardening..."
...
########################################
# STEP 6: DISABLE MODULES
########################################
echo "[6/15] Disable kernel modules..."
...
########################################
# STEP 7: PASSWORD + AUTH
########################################
echo "[7/15] Password & auth..."
...
for user in daemon bin sys games man lp mail news uucp proxy www-data backup list irc gnats nobody; do
  id "$user" >/dev/null 2>&1 && usermod -s /usr/sbin/nologin "$user" 2>/dev/null && passwd -l "$user" 2>/dev/null
done
usermod -s /bin/bash root 2>/dev/null

# PAM
[ -f /etc/pam.d/common-password ] && {
  grep -q "remember=" /etc/pam.d/common-password || sed -i '/pam_unix.so/ s/$/ remember=5 sha512/' /etc/pam.d/common-password
}
[ -f /etc/pam.d/common-auth ] && {
  grep -q "pam_faildelay" /etc/pam.d/common-auth || echo "auth optional pam_faildelay.so delay=4000000" >> /etc/pam.d/common-auth
}
...
# Restrict su
[ -f /etc/pam.d/su ] && {
  grep -q "pam_wheel.so" /etc/pam.d/su || sed -i '/pam_rootok/a auth required pam_wheel.so use_uid' /etc/pam.d/su
}

echo "  Done ✔"

########################################
# STEP 8: SHELL HARDENING
########################################
echo "[8/15] Shell hardening..."
...
########################################
# STEP 12: LOGGING + CORE DUMPS + SUDO
########################################
echo "[12/15] Logging, core dumps, sudo..."
...
########################################
# STEP 13: AIDE + RKHUNTER + RESCUE
########################################
echo "[13/15] Security tools..."

# AIDE
if command -v aideinit >/dev/null 2>&1; then
  echo "  Building AIDE database (takes a minute)..."
...
########################################
# STEP 14: HOSTNAME + DNS + SCHEDULER
########################################
echo "[14/15] Hostname, DNS, scheduler..."
...
# Systemd service hardening (safe ones only - NOT ssh)
for svc in cron rsyslog chrony fail2ban unattended-upgrades; do
  mkdir -p /etc/systemd/system/${svc}.service.d
  cat > /etc/systemd/system/${svc}.service.d/hardening.conf <<'SVCEOF'
[Service]
ProtectSystem=full
PrivateTmp=true
...
# Restart hardened services and verify
for svc in cron rsyslog chrony fail2ban; do
...
########################################
# STEP 15: LYNIS + DEBIAN-CIS
########################################
echo "[15/15] Lynis + debian-cis..."
...
echo -n "  Internet: "
ping -c 1 -W 3 1.1.1.1 >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  DNS:      "
ping -c 1 -W 3 google.com >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  APT:      "
apt-get update -qq >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  SSH:      "
systemctl is-active ssh >/dev/null 2>&1 && echo "✔ (port ${SSH_PORT})" || echo "✘"

echo -n "  Firewall: "
ufw status 2>/dev/null | grep -q "active" && echo "✔" || echo "✘"

echo -n "  Fail2ban: "
systemctl is-active fail2ban >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  Auditd:   "
systemctl is-active auditd >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  AppArmor: "
systemctl is-active apparmor >/dev/null 2>&1 && echo "✔" || echo "✘"

echo -n "  AIDE:     "
[ -f /var/lib/aide/aide.db ] && echo "✔" || echo "building..."
